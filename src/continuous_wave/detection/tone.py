...
class EnvelopeDetector(ToneDetector):
    ...
    config: CWConfig
    _envelope_filter: Optional[tuple] = field(default=None, init=False)
    _zi: Optional[NDArray[np.float64]] = field(default=None, init=False)
    ...
    def __post_init__(self) -> None:
        ...
        cutoff_hz = 50.0
        nyquist = self.config.audio.sample_rate / 2
        normalized_cutoff = cutoff_hz / nyquist

        # 4th order Butterworth lowpass
        sos = sp_signal.butter(
            4, normalized_cutoff, btype="low", output="sos", fs=self.config.audio.sample_rate
        )
        self._envelope_filter = sos
        self._zi = sp_signal.sosfilt_zi(sos)

        # Initialize threshold based on config squelch
        self._threshold = self.config.squelch.threshold
        self._hysteresis = self.config.squelch.hysteresis
    ...
    def detect(self, audio: NDArray[np.float64]) -> list[ToneEvent]:
        ...
        envelope, self._zi = sp_signal.sosfilt(
